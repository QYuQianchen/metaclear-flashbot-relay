...
var Migration001InitDatabase = &migrate.Migration{
	...
	Up: []string{`
		CREATE TABLE IF NOT EXISTS ` + vars.TableMetadata + ` (
		...
		CREATE UNIQUE INDEX IF NOT EXISTS ` + vars.TableMetadata + `_method_key_timestamp_uidx ON ` + vars.TableMetadata + `(method, key, received_at DESC);
		
		CREATE TABLE IF NOT EXISTS ` + vars.TableProposerDuties + ` (
			id          		bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
			inserted_at 		timestamp NOT NULL default current_timestamp,
		...
		CREATE UNIQUE INDEX IF NOT EXISTS ` + vars.TableProposerDuties + `_slot_idx ON ` + vars.TableProposerDuties + `("slot");
		
		CREATE TABLE IF NOT EXISTS ` + vars.TablePayloadAttributes + ` (
			id          		bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
			inserted_at 		timestamp NOT NULL default current_timestamp,
			slot				bigint NOT NULL,
			parent_hash			text NOT NULL
			withdrawals_root	text NOT NULL
			parent_beacon_root	text NOT NULL
			prev_randao			text NOT NULL
			timestamp			bigint NOT NULL
		);

		CREATE UNIQUE INDEX IF NOT EXISTS ` + vars.TablePayloadAttributes + `_slot_hash_uidx ON ` + vars.TablePayloadAttributes + `(slot, parent_hash);
	...
	Down: []string{`
		...
		DROP TABLE IF EXISTS ` + vars.TableMetadata + `;
		`},
